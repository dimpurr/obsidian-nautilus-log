...
你的工作目录是 `/Users/dimpurr/scratch/naut-tri/host`（隔离 git worktree，分支 `t/host`）。所有命令在那里跑。
...
`src/main.ts` · `src/settings.ts` · `src/parser.ts` · `src/contract.ts` · 对应 test

典型条目：L1-003/004/039/040（设置面板绕过引擎常量与 boundedInteger 夹取）· L2-034/079 · T2-061/098/108 · E1 与 G1 里落在宿主的。
...
- **不要 commit、不要 push。**
- 报告写进 `/Users/dimpurr/scratch/naut-tri/report-host.md`：一张表 `| 原ID | 档 | 处置 | 证据 file:line |`，
