...
你的工作目录是 `/Users/dimpurr/scratch/naut-tri/data`（隔离 git worktree，分支 `t/data`）。所有命令在那里跑。
...
`src/timing-obsidian.ts` · `test/timing-writeback.test.js`

典型条目：A1-003/005/009/011/044/049/066/069/083/084 以及 recert-6 里其余落在数据层的。⚠️ A1-066（CRLF 两处切分不一致）主会话已核实【属实】。
...
- **不要 commit、不要 push。**
- 报告写进 `/Users/dimpurr/scratch/naut-tri/report-data.md`：一张表 `| 原ID | 档 | 处置 | 证据 file:line |`，
